<script>
    ...
    const days = ["all", ...new Set(data.flatMap(item => item.day)).values()].sort((a, b) => a - b)

    const filtered = $derived(
        data.filter(item => {
            const wingMatch = selectedWing === "all" || item.wing.includes(selectedWing)
            const dayMatch = selectedDay === "all" || item.day.includes(Number(selectedDay))
        ...
        >
            {#each wings as wing}
                <option class="text-(--text-white)" value={wing}>
                    {wing === "all" ? "All Wings" : wing.charAt(0).toUpperCase() + wing.slice(1)}
                </option>
            {/each}
        </select>
    </div>
