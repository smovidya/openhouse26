...
export function getProviceCode(name: string) {
  return provinces.find(it => it.provinceNameEn === name || it.provinceNameTh === name)?.provinceCode;
}

export function getProviceByCode(code: number) {
  return provinceMap.get(code);
}

export const provincesOptions = provinces.map(it => ({
  label: it.provinceNameTh,
  value: it.provinceCode,
}));

export const provinceMap = new Map<number, Province>();
for (const province of provinces) {
  provinceMap.set(province.provinceCode, province);
}
