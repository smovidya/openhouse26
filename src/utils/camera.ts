...
export function getUsableMediaDevices(mediaDevices: MediaDeviceInfo[]) {
  // blink on android
  let usables = mediaDevices.filter((it) =>
    blinkCameraLabelPattern.test(it.label),
  );
  if (usables.length === 0) {
    // gecko on android
    usables = mediaDevices.filter((it) =>
      geckoCameraLabelPattern.test(it.label),
    );
  }
  ...
  if (usables.length > 0) {
    ...
    const other = mediaDevices.filter(
      (it) => !blinkCameraLabelPattern.test(it.label),
    );
    const sorted = mediaDevices.toSorted((a, b) =>
